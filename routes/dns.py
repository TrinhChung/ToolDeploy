...
from util.cloud_flare import add_dns_record, get_dns_records, delete_dns_record_cf
...
def sync_dns(domain_id):
    ...
    domain = Domain.query.get_or_404(domain_id)
    try:
        get_dns_records(domain.zone_id)
...
def add_dns(domain_id):
    ...
    if request.method == "POST":
        ...
        try:
            add_dns_record(domain.zone_id, name, content, record_type, ttl, proxied)
...
def delete_dns(record_id):
    ...
    try:
        # Xóa ở Cloudflare (nếu muốn)
        if record.record_id:
            delete_dns_record_cf(domain.zone_id, record.record_id)
        # Xóa ở local DB
